    ...
    useEffect(() => {
        const fetchInitialData = async () => {
            try {
                ...
                if (id) {
                    const movieRes = await api.movie(id); // assuming standard GET movie returns full detail
            ...
            <form onSubmit={handleSubmit} className="bg-[#151B27] border border-white/10 rounded-xl p-6 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} className="w-full bg-[#101521] border border-white/10 rounded-lg px-3 py-2 text-white focus:border-[#E50914] outline-none" placeholder="VD: 120 phút" />
                    </div>
